...
오시는 길 섹션의 인터랙티브 지도. 키가 없으면 "지도를 불러오는 중…" 상태로 유지됨 (graceful fallback).

### 키 발급 (5분, 무료)
1. https://developers.kakao.com/console/app 접속 (카카오 계정 로그인)
2. **애플리케이션 추가** → 이름 자유 (예: `wedding-card`)
3. 생성된 앱 → **앱 키** 탭에서 **JavaScript 키** 복사
4. 좌측 메뉴 **플랫폼 → Web 플랫폼 등록**
   - 사이트 도메인:
     - 개발용: `http://localhost:8080`
     - 배포용: `https://wonjjong.github.io`
     - 한 줄에 하나씩, 둘 다 추가
5. 좌측 메뉴 **제품 설정 → 카카오맵** → **ON**

### HTML에 적용
`index.html` 의 SDK 스크립트 부분에서:
```html
<script src="//dapi.kakao.com/v2/maps/sdk.js?appkey=KAKAO_JS_KEY&libraries=services&autoload=false"></script>
```
`KAKAO_JS_KEY` 자리에 복사한 키를 그대로 붙여넣고 push.

### 참고
- 키가 노출돼도 도메인 등록 덕분에 다른 사이트에선 사용 불가
- 무료 한도: 일 30만 회 (청첩장엔 충분)
